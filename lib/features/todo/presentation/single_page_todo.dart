...
class SyncEngine {
  ...
  Future<void> _handleIncoming(TodoEvent event) async {
    final incoming = event.todo;

    // Handle delete events
    if (event.type == "delete") {
      await localStore.deleteTodo(incoming.id);
      await _refreshLocalNotifier();
      debugPrint(
        "$deviceId: applied remote delete for ${incoming.id} from ${event.originDeviceId}",
      );
      return;
    }
    ...
    final existing = all.firstWhere(
      (e) => e.id == incoming.id,
      orElse: () =>
          Todo(id: incoming.id, title: '', updatedBy: incoming.updatedBy),
    );
    ...
    if (shouldApply) {
      debugPrint(
        "$deviceId: applying remote change for ${incoming.id} from ${event.originDeviceId}",
      );
      await localStore.saveTodo(incoming);
      await _refreshLocalNotifier();
    } else {
      debugPrint("$deviceId: ignored remote change for ${incoming.id} (stale)");
    }
  }
  ...
  Future<void> _refreshLocalNotifier() async {
    ...
    localTodosNotifier.value = data.where((t) => !t.isDeleted).toList();
  }
