...
class SyncEngine {
  ...
  Future<void> onEdit(Todo t) async {
  ...
  Future<void> _handleIncoming(TodoEvent event) async {
    ...
    final existing = all.firstWhere(
      ...
      orElse: () => Todo(
          id: incomingTodo.id, title: '', updatedBy: incomingTodo.updatedBy),
    );
    ...
    if (shouldApply) {
      ...
      debugPrint(
          "$deviceId: ignored remote change for ${incomingTodo.id} (stale)");
    }
  }
...
class _DevicePanelState extends State<DevicePanel> {
  ...
  Future<void> _createTodo() async {
    ...
    await widget.syncEngine.onEdit(t);
  }
  ...
  Future<void> _editTodoInline(Todo todo) async {
    ...
    if (result != null && result.isNotEmpty) {
      ...
      await widget.syncEngine.onEdit(updated);
    }
  }
